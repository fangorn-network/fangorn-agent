...
import { Manifest, FileEntry } from "../../types/subgraph";
...
/* ── ManifestContentCard ── */
interface ManifestContentCardProps { manifest: Manifest; index: number; isExpanded: boolean; onToggle: () => void; }
const FILES_PER_PAGE = 5;

export const ManifestContentCard = ({ manifest, index, isExpanded, onToggle }: ManifestContentCardProps) => {
  const [hovered, setHovered] = useState(false);
            ...
            <>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {visibleFiles.map((file, i) => {
                  ...
                  return <FileRow key={file.id || globalIdx} file={file} fileIndex={globalIdx} isSelected={selectedFile === globalIdx} onSelect={() => setSelectedFile(selectedFile === globalIdx ? null : globalIdx)} />;
                })}
              </div>
              {totalFilePages > 1 && (
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8, marginTop: 8, fontSize: 11, color: "var(--color-text-secondary, #8a8a8a)" }}>
                  <button onClick={() => { setFilePage((p) => Math.max(1, p - 1)); setSelectedFile(null); }} disabled={filePage <= 1} style={{ border: "none", background: "none", cursor: filePage <= 1 ? "default" : "pointer", color: filePage <= 1 ? "var(--color-text-tertiary, #5a5a5a)" : "var(--color-text-primary, #fafafa)", fontSize: 13, fontWeight: 600, padding: "2px 6px" }}>‹</button>
                  <span>{filePage} / {totalFilePages}</span>
                  <button onClick={() => { setFilePage((p) => Math.min(totalFilePages, p + 1)); setSelectedFile(null); }} disabled={filePage >= totalFilePages} style={{ border: "none", background: "none", cursor: filePage >= totalFilePages ? "default" : "pointer", color: filePage >= totalFilePages ? "var(--color-text-tertiary, #5a5a5a)" : "var(--color-text-primary, #fafafa)", fontSize: 13, fontWeight: 600, padding: "2px 6px" }}>›</button>
                </div>
              )}
            </>
          )}
