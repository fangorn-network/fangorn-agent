...
import { ManifestState, FileEntry } from "../../types/subgraph";
...
/* ── File entry row ── */
interface FileEntryRowProps { file: FileEntry; fileIndex: number; isSelected: boolean; onSelect: () => void; }
...
const FileEntryRow = ({ file, fileIndex, isSelected, onSelect }: FileEntryRowProps) => {
  ...
  const totalEncPrice = encFields.reduce((sum, f) => sum + (f.price != null ? Number(f.price.price) : 0), 0);
  ...
  return (
    <div onClick={(e) => { e.stopPropagation(); onSelect(); }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      {isSelected && (
        <div style={{ marginTop: 8, borderTop: "0.5px solid var(--color-border-tertiary, #1e1e1e)", paddingTop: 6 }} onClick={(e) => e.stopPropagation()}>
          {file.fields.map((f) => (
            <div key={f.id || f.name}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "3px 0", fontSize: 11, gap: 8 }}>
                ...
                <div style={{ textAlign: "right", maxWidth: "55%", wordBreak: "break-all" }}>
                  {f.acc === "plain" ? <span style={{ color: "var(--color-text-primary, #fafafa)", fontWeight: 500 }}>{f.value}</span> : <EncryptedBadge />}
                </div>
              </div>
          ...
          <div style={{ marginTop: 6, paddingTop: 6, borderTop: "0.5px solid var(--color-border-tertiary, #1e1e1e)" }}>
            <div style={{ display: "flex", gap: 6, alignItems: "center", background: "var(--color-background-primary, #141414)", border: "0.5px solid var(--color-border-tertiary, #1e1e1e)", borderRadius: 7, padding: 3 }}>
              <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleChatSubmit()} placeholder="Ask about this file..."
              ...
              <button onClick={handleChatSubmit} disabled={!chatInput.trim()} style={{ border: "none", background: chatInput.trim() ? "var(--color-text-primary, #fafafa)" : "var(--color-border-tertiary, #1e1e1e)", color: chatInput.trim() ? "var(--color-background-primary, #141414)" : "var(--color-text-tertiary, #5a5a5a)", borderRadius: 5, padding: "3px 8px", fontSize: 10, fontWeight: 600, cursor: chatInput.trim() ? "pointer" : "default", transition: "background 0.15s, color 0.15s", fontFamily: "var(--font-body, sans-serif)" }}>↵</button>
            </div>
          </div>
        </div>
      )}
...
/* ── ManifestCard ── */
interface ManifestCardProps { index: number; manifestState: ManifestState; isExpanded: boolean; onToggle: () => void; selectedFileIndex: number | null; onFileSelect: (fileIndex: number | null) => void; }
const FILES_PER_PAGE = 5;

export const ManifestCard = ({ index, manifestState, isExpanded, onToggle, selectedFileIndex, onFileSelect }: ManifestCardProps) => {
  const [hovered, setHovered] = useState(false);
  const [filePage, setFilePage] = useState(1);
  const [chatInput, setChatInput] = useState("");
  const [hasSent, setHasSent] = useState(false);
  const { sendMessage } = useChatContext();
  const contextLabel = `Re: ${manifestState.schema_name} (${truncAddr(manifestState.owner)})`;

  const handleChatSubmit = () => {
    if (!chatInput.trim()) return;
    const context = { id: manifestState.id, owner: manifestState.owner, schema_name: manifestState.schema_name, schema_id: manifestState.schema_id, version: manifestState.version, manifest_cid: manifestState.manifest_cid, fileCount: manifestState.manifest?.files?.length || 0 };
    sendMessage(`In regards to this ManifestState ${JSON.stringify(context)}: ${chatInput}`, { contextLabel, contextType: "manifest", displayMessage: chatInput });
    setChatInput("");
    setHasSent(true);
    onToggle(); // collapse
  };
    ...
    <div onClick={onToggle} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ maxWidth: "65%" }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: "var(--color-text-primary, #fafafa)", fontFamily: "var(--font-mono, monospace)" }}>{manifestState.schema_name || `Manifest ${index + 1}`}</div>
          <div style={{ fontSize: 11, color: "var(--color-text-tertiary, #5a5a5a)", marginTop: 3, fontFamily: "var(--font-mono, monospace)" }}>owner: {truncAddr(manifestState.owner)}</div>
        </div>
        <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
      ...
      <div style={{ fontSize: 11, color: "var(--color-text-tertiary, #5a5a5a)", marginTop: 3 }}>
        {manifestState.version}{manifestState.manifest_cid && <span> · CID: {truncAddr(manifestState.manifest_cid)}</span>}
      </div>
      {isExpanded && (
        <div style={{ marginTop: 10, borderTop: "0.5px solid var(--color-border-tertiary, #1e1e1e)", paddingTop: 8 }} onClick={(e) => e.stopPropagation()}>
          <div style={{ fontSize: 12, fontWeight: 600, color: "var(--color-text-primary, #fafafa)", marginBottom: 6 }}>File Entries</div>
          {fileCount === 0 ? (
            ...
            <>
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {visibleFiles.map((file, i) => {
                  ...
                  return <FileEntryRow key={file.id || globalFileIdx} file={file} fileIndex={globalFileIdx} isSelected={selectedFileIndex === globalFileIdx} onSelect={() => onFileSelect(selectedFileIndex === globalFileIdx ? null : globalFileIdx)} />;
                })}
              </div>
              {totalFilePages > 1 && (
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 8, marginTop: 8, fontSize: 11, color: "var(--color-text-secondary, #8a8a8a)" }}>
                  <button onClick={() => { setFilePage((p) => Math.max(1, p - 1)); onFileSelect(null); }} disabled={filePage <= 1} style={{ border: "none", background: "none", cursor: filePage <= 1 ? "default" : "pointer", color: filePage <= 1 ? "var(--color-text-tertiary, #5a5a5a)" : "var(--color-text-primary, #fafafa)", fontSize: 13, fontWeight: 600, padding: "2px 6px" }}>‹</button>
                  <span>{filePage} / {totalFilePages}</span>
                  <button onClick={() => { setFilePage((p) => Math.min(totalFilePages, p + 1)); onFileSelect(null); }} disabled={filePage >= totalFilePages} style={{ border: "none", background: "none", cursor: filePage >= totalFilePages ? "default" : "pointer", color: filePage >= totalFilePages ? "var(--color-text-tertiary, #5a5a5a)" : "var(--color-text-primary, #fafafa)", fontSize: 13, fontWeight: 600, padding: "2px 6px" }}>›</button>
                </div>
              )}
            </>
          )}
          <div style={{ marginTop: 8, paddingTop: 8, borderTop: "0.5px solid var(--color-border-tertiary, #1e1e1e)" }}>
            <div style={{ display: "flex", gap: 6, alignItems: "center", background: "var(--color-background-secondary, #0e0e0e)", border: "0.5px solid var(--color-border-tertiary, #1e1e1e)", borderRadius: 8, padding: 4 }}>
              <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleChatSubmit()} placeholder="Ask about this manifest..."
